...
# This script is a modified version of the react-native-xcode.sh script that is
# used to bundle React Native apps. The main difference is that this script will
# bundle the app for the widget extension target instead of the main app target.
# This script is invoked by Xcode as part of the build process.

# Since we don't have a way to pass bundle name to this script, we replace main.jsbundle
...
BUNDLE_FILE="$CONFIGURATION_BUILD_DIR/widget.jsbundle"
...
if [[ $USE_HERMES == false ]]; then
  cp "$BUNDLE_FILE" "$DEST/"
  BUNDLE_FILE="$DEST/widget.jsbundle"
else
  EXTRA_COMPILER_ARGS=
  if [[ $DEV == true ]]; then
  ...
  "$HERMES_CLI_PATH" -emit-binary -max-diagnostic-width=80 $EXTRA_COMPILER_ARGS -out "$DEST/widget.jsbundle" "$BUNDLE_FILE"
  if [[ $EMIT_SOURCEMAP == true ]]; then
    HBC_SOURCEMAP_FILE="$DEST/widget.jsbundle.map"
